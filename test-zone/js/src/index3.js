...
const stringToIntParser = params => Number(params.newValue);

const createColumnDefs = () => [
    { field: 'athlete', checkboxSelection: true },
    { field: 'country' },
    { field: 'year', width: 100 },
    { field: 'gold', width: 100, cellRenderer: 'myCellRenderer', valueParser: stringToIntParser },
    { field: 'silver', width: 100, cellRenderer: 'myCellRenderer', valueParser: stringToIntParser },
    { field: 'bronze', width: 100, cellRenderer: 'myCellRenderer', valueParser: stringToIntParser },
...
// Function for computing the Total

function addTotalColumn() {
    gridOpts.columnDefs.push({
        field: 'total',
        valueGetter: 'data.gold + data.silver + data.bronze',
        colId: 'total',
    });
    gridOpts.api.setColumnDefs(gridOpts.columnDefs);
    // gridOpts.api.refreshClientSideRowModel('aggregate');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Adding a Total Column of rows

    const buttonTotalColumn = document.createElement('button');
    buttonTotalColumn.appendChild(document.createTextNode('Add Total Column'));
    buttonTotalColumn.addEventListener('click', () => addTotalColumn());
    gridDiv.parentNode.insertBefore(buttonTotalColumn, gridDiv);
